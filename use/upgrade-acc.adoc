...
NOTE: Eine Änderung der Astra-Version ist die einzige Voraussetzung für ein Upgrade des Astra Control Center. Ihr Registrierungspfad muss mit dem Registrierungspfad übereinstimmen, in dem Sie die Bilder in A verschoben haben ,Vorheriger Schritt.
...
NOTE: Dies muss bereits geschehen  up acc-helm-repo to upgrade Traefik and Cert-manager,ACC-Helm-Repo einrichten Vor dem Abschluss des folgenden Verfahrens.
...
NOTE: Sie müssen bereits den abgeschlossen haben  Traefik service using acc-helm-repo,Traefik-Update Und  up acc-helm-repo to upgrade Traefik and Cert-manager,ACC-Helm-repo in Helm hinzugefügt Vor dem Abschluss des folgenden Verfahrens.
